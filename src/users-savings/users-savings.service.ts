...
export class UsersSavingsService {
    ...
    async approvePrincipalSavingsForUser(
        ...
        const principalSavings = await this.savingsRepository
            .findPrincipalSavingsByUserId(userId)
    ...
    async createPrincipalSavings(userId: string, trx?: Knex.Transaction): Promise<void> {
        ...
        this.logger.log(`Principal savings created for user ${userId} with amount ${amount}`)
    }
     ...
     */
    private async calculatePrincipalSavingsAmount(trx?: Knex.Transaction): Promise<number> {
        const totalBalance = await this.savingsRepository.getCashbookBalance(trx)
        const activeMemberCount = await this.usersRepository.getActiveMemberCount(trx)
