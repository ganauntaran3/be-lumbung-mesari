...
describe('UsersRepository', () => {
    ...
    describe('findByEmailWithRole', () => {
        it('should find user with role by email', async () => {
            ...
            expect(mockKnex.join).toHaveBeenCalledWith('roles', 'roles.id', 'users.role_id')
            expect(mockKnex.where).toHaveBeenCalledWith('users.email', 'test@example.com')
        })
    })
    ...
    describe('findByIdentifierWithRole', () => {
        it('should find user by email or username', async () => {
            ...
            const result = await repository.findByIdentifierWithRole('test@example.com')

            expect(result).toEqual(mockUserWithRole)
            expect(mockKnex.where).toHaveBeenCalledWith('users.email', 'test@example.com')
            expect(mockKnex.orWhere).toHaveBeenCalledWith('users.username', 'test@example.com')
        })
    })
    ...
    describe('createUser', () => {
        it('should create a new user', async () => {
            ...
            const createdUser = { id: '1', ...newUserData, created_at: new Date(), updated_at: new Date() }
