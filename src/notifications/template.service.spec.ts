...
describe('TemplateService', () => {
  ...
  it('should validate template existence', () => {
    expect(
      service.templateExists(NotificationTemplate.REGISTRATION_CONFIRMATION)
    ).toBe(true)
    expect(
      service.templateExists(NotificationTemplate.REGISTRATION_APPROVED)
    ).toBe(true)
    expect(
      service.templateExists('non_existent_template' as NotificationTemplate)
    ).toBe(false)
  })
  ...
  it('should handle conditional blocks in templates', () => {
    ...
    const renderedWithReason = service.renderTemplate(
      NotificationTemplate.REGISTRATION_REJECTED,
      templateDataWithReason
    )
    const renderedWithoutReason = service.renderTemplate(
      NotificationTemplate.REGISTRATION_REJECTED,
      templateDataWithoutReason
    )
  ...
  it('should generate correct subject for templates', () => {
    ...
    const subject1 = service.getTemplateSubject(
      NotificationTemplate.REGISTRATION_CONFIRMATION,
      templateData
    )
    const subject2 = service.getTemplateSubject(
      NotificationTemplate.REGISTRATION_APPROVED,
      templateData
    )

    expect(subject1).toBe(
      'Selamat datang di Lumbung Mesari - Konfirmasi Pendaftaran'
    )
  ...
  it('should generate text version from HTML', () => {
    const html =
      '<h1>Hello World</h1><p>This is a <strong>test</strong> email.</p>'
  ...
  it('should throw error for non-existent template', () => {
    ...
    expect(() => {
      service.renderTemplate(
        'non_existent' as NotificationTemplate,
        templateData
      )
