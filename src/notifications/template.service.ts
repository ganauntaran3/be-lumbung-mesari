...
export class TemplateService {
    ...
    try {
      ...
      for (const file of templateFiles) {
        if (file.endsWith('.hbs')) {
          ...
          this.logger.debug(
            `✅ Template compiled successfully: ${templateName}`
          )
        }
      }

      this.logger.log(
        '🔍 DEBUG: All compiled templates:',
        Array.from(this.compiledTemplates.keys())
      )
    } catch (error) {
  ...
  getTemplateSubject(
    ...
    const subjects: Record<NotificationTemplate, string> = {
      ...
      [NotificationTemplate.REGISTRATION_APPROVED]:
        'Pendaftaran Disetujui - Lumbung Mesari',
      [NotificationTemplate.REGISTRATION_REJECTED]:
        'Pendaftaran Perlu Diperbaiki - Lumbung Mesari',
      [NotificationTemplate.ADMIN_NEW_REGISTRATION]:
        'Pendaftaran Anggota Baru Menunggu Persetujuan',
      [NotificationTemplate.PROFILE_UPDATED]:
        'Profil Diperbarui - Lumbung Mesari',
      [NotificationTemplate.PASSWORD_RESET]: 'Reset Password - Lumbung Mesari',
      [NotificationTemplate.ACCOUNT_SUSPENDED]:
        'Akun Disuspend - Lumbung Mesari',
      [NotificationTemplate.OTP_VERIFICATION]:
        'Kode Verifikasi OTP - Lumbung Mesari'
    }
  ...
  private getCooperativeAddress(): string {
    return (
      process.env.COOPERATIVE_ADDRESS ||
