...
export class EmailHelperService {
  ...
  async sendEmail(emailData: EmailData): Promise<void> {
    try {
      ...
      const subject = this.templateService.getTemplateSubject(
        emailData.template,
        emailData.data
      )
      ...
      this.logger.log(
        `Email sent successfully to ${emailData.recipient} with template ${emailData.template}`
      )
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error'
      this.logger.error(
        `Failed to send email to ${emailData.recipient}: ${errorMessage}`,
        error
      )
      throw error
    }
