...
export async function seed(knex: Knex): Promise<void> {
  ...
  if (!hasTable) {
    console.log(
      '⚠️  income_categories table does not exist yet. Skipping seed.'
    )
    return
  }
  ...
  await knex('income_categories').insert([
    {
      ...
      name: 'Simpanan Pokok',
      description:
        'Pendapatan dari simpanan pokok anggota baru yang bergabung dengan koperasi',
    ...
    },
    {
      ...
      description:
        'Pendapatan dari sumber lain yang tidak termasuk kategori di atas',
      default_destination: 'capital'
    }
  ])
