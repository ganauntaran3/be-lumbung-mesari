...
export class SavingsQueryDto extends PaginationQueryDto {
  @ApiPropertyOptional({
    description:
      'Filter by period (month name in English) - uses current year by default',
    enum: [
      'january',
      'february',
      'march',
  ...
  @Matches(
    ...
    {
      message:
        'Period must be a valid month name in English (e.g., january, february, etc.)'
    }
  )
  period?: string
  ...
  @ApiPropertyOptional({
    description:
      'Filter by year (e.g., 2025, 2026). Use with period parameter to get specific month/year combination. If not provided, uses current year.',
    example: 2025,
    type: Number
  })
  @IsOptional()
