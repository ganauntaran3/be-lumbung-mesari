...
export class PrincipalSavingsService {
  ...
  async createPrincipalSavings(
    ...
    try {
      ...
      this.logger.log(
        `Creating principal savings for user ${userId} with calculated amount ${principalAmount}`
      )
      ...
      this.logger.log(
        `Principal savings created successfully for user ${userId} with amount ${principalAmount}`
      )

      // Fetch with user info
      return await this.savingsRepository.findPrincipalSavingsByIdWithUser(
        principalSavings.id
      )
    } catch (error) {
      this.logger.error(
        `Failed to create principal savings for user ${userId}:`,
        error
      )
      throw error
    }
  ...
  async approvePrincipalSavings(
    ...
    try {
      this.logger.log(
        `Approving principal savings for user ${userId} by admin ${processedBy}`
      )
      ...
      if (!principalSavings) {
        throw new NotFoundException(
          `Principal savings not found for user ${userId}`
        )
      }
      ...
      const updated = await this.savingsRepository.updatePrincipalSavings(
        principalSavings.id,
        {
          status: 'paid',
      ...
      this.logger.log(
        `Cashbook transaction created. Principal savings approval complete for user ${userId}`
      )
      ...
      this.logger.error(
        `Failed to approve principal savings for user ${userId}:`,
        error
      )
      throw error
    }
  ...
  private async calculatePrincipalSavingsAmount(): Promise<number> {
    try {
      ...
      if (principalAmount < minAmount) {
        this.logger.warn(
          `Calculated amount ${principalAmount} is below minimum ${minAmount}. Using minimum.`
        )
        return minAmount
      }

      this.logger.log(
        `Calculated principal savings: ${totalBalance} / ${activeMemberCount} = ${principalAmount}`
      )
  ...
  private getMinimumPrincipalSavingsAmount(): number {
    const amount = this.configService.get<number>(
      'PRINCIPAL_SAVINGS_MIN_AMOUNT',
      50000
    )

    if (amount <= 0) {
      this.logger.warn(
        `Invalid minimum principal savings amount configured: ${amount}. Using default: 50000`
      )
      return 50000
    }
