...
export class CashbookTransactionRepository extends BaseRepository<CashbookTransactionTable> {
    ...
    async createTransaction(
        ...
        try {
            ...
            this.logger.log(`Cashbook transaction created: ${result.id} (${data.direction} ${data.amount})`)
     ...
     */
    async getTransactionsWithFilters(filters: TransactionFilters = {}): Promise<CashbookTransactionTable[]> {
        try {
            this.logger.debug('Getting transactions with filters:', JSON.stringify(filters))
     ...
     */
    async getTransactionsByIncome(incomeId: string): Promise<CashbookTransactionTable[]> {
        try {
            return await this.getTransactionsWithFilters({ incomeId })
        } catch (error) {
            this.logger.error(`Failed to get transactions for income ${incomeId}:`, error)
            throw error
        }
    }
     ...
     */
    async getTransactionsByExpense(expenseId: string): Promise<CashbookTransactionTable[]> {
        try {
            return await this.getTransactionsWithFilters({ expenseId })
        } catch (error) {
            this.logger.error(`Failed to get transactions for expense ${expenseId}:`, error)
            throw error
        }
    }
     ...
     */
    async getTransactionCount(filters: Omit<TransactionFilters, 'limit' | 'offset'> = {}): Promise<number> {
        try {
            let query = this.knex('cashbook_transactions')
     ...
     */
    async getTransactionSummary(filters: Omit<TransactionFilters, 'limit' | 'offset'> = {}): Promise<{
        totalIncome: number
        totalExpense: number
        netFlow: number
