    ...
    describe('/users (GET) - User Management Tests', () => {
        ...
        it('should return users ordered by created_at DESC by default', async () => {
            ...
            if (response.body.data.length > 1) {
                const createdDates = response.body.data.map((user: any) => new Date(user.created_at))
        ...
        it('should support custom sorting', async () => {
            ...
            const fullnames = nameDescResponse.body.data.map((user: any) => user.fullname)
        ...
        it('should handle different user statuses', async () => {
            ...
            const validStatuses = ['waiting_deposit', 'active', 'suspended', 'pending']
