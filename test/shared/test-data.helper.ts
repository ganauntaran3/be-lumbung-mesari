...
export class TestDataHelper {
    ...
    private static roles: { memberRoleId: string; adminRoleId: string } | null = null
    ...
    static async setupSharedTestData(databaseService: DatabaseService): Promise<{
        ...
        try {
            ...
            let adminRoleId = existingRoles.find(r => r.name === 'administrator')?.id
            ...
            const testUserCount = parseInt(existingTestUsers?.count as string || '0')

            if (testUserCount < 25) {
                console.log(`📝 Creating ${25 - testUserCount} additional test users...`)
                ...
                const testUsers = Array.from({ length: 25 - testUserCount }, (_, i) => {
                    ...
                    return {
                        ...
                        created_at: new Date(`2024-01-${String((userIndex % 30) + 1).padStart(2, '0')}`),
                        updated_at: new Date(`2024-01-${String((userIndex % 30) + 1).padStart(2, '0')}`)
                    }
                })
    ...
    static async cleanupAllTestData(databaseService: DatabaseService): Promise<void> {
        try {
            const knex = databaseService.getKnex()
